    ...
    def connect(self):
        ...
        try:
            if not isinstance(self.capabilities, dict):
                raise ValueError("Capabilities должны быть словарем.")
            ...
            logger.error(f"Ошибка подключения к Android TV: {e}. Возможно, некорректный формат capabilities.")
        except Exception as e:
            logger.error(f"Общая ошибка подключения к Android TV: {e}")
            raise
